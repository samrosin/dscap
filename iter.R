...
source(paste(wd, "estFUN_taudelta.R",sep="")) # store this file in the wd
source(paste(wd, "parameter_values.R",sep="")) # also store this file in the wd
...
beta <- coef(lm(tau ~ delta))


# bootstrap for rho_s confidence intervals --------------------------------
df1 <- df %>% filter(trial == 1)
df2 <- df %>% filter(trial == 2)
df3 <- df %>% filter(trial == 3)
df4 <- df %>% filter(trial == 4)
df5 <- df %>% filter(trial == 5)

rho_s_boot <- rep(NA, n_boot)
for(i in 1:n_boot){
  
  df1_boot <- df1[sample(nrow(df1), nrow(df1), replace=T),]
  df2_boot <- df2[sample(nrow(df2), nrow(df2), replace=T),]
  df3_boot <- df3[sample(nrow(df3), nrow(df3), replace=T),]
  df4_boot <- df4[sample(nrow(df4), nrow(df4), replace=T),]
  df5_boot <- df5[sample(nrow(df5), nrow(df5), replace=T),]
  df_boot <- rbind(df1_boot, df2_boot, df3_boot, df4_boot, df5_boot)
  
  # outcome models E[O | X=x, A=a]
  model_y0 <- glm(Ymod, data=subset(df_boot, A==0), family = binomial)
  model_y2 <- glm(Ymod, data=subset(df_boot, A==2), family = binomial)
  model_y3 <- glm(Ymod, data=subset(df_boot, A==3), family = binomial)
  model_y4 <- glm(Ymod, data=subset(df_boot, A==4), family = binomial)
  model_y5 <- glm(Ymod, data=subset(df_boot, A==5), family = binomial)
  
  model_s0 <- glm(Smod, data=subset(df_boot, A==0), family = gaussian)
  model_s2 <- glm(Smod, data=subset(df_boot, A==2), family = gaussian)
  model_s3 <- glm(Smod, data=subset(df_boot, A==3), family = gaussian)
  model_s4 <- glm(Smod, data=subset(df_boot, A==4), family = gaussian)
  model_s5 <- glm(Smod, data=subset(df_boot, A==5), family = gaussian)
  
  df_boot$py0 <- predict(model_y0,newdata=df_boot, type="response")
  df_boot$py2 <- predict(model_y2,newdata=df_boot, type="response")
  df_boot$py3 <- predict(model_y3,newdata=df_boot, type="response")
  df_boot$py4 <- predict(model_y4,newdata=df_boot, type="response")
  df_boot$py5 <- predict(model_y5,newdata=df_boot, type="response")
  
  df_boot$ps0 <- predict(model_s0,newdata=df_boot, type="response")
  df_boot$ps2 <- predict(model_s2,newdata=df_boot, type="response")
  df_boot$ps3 <- predict(model_s3,newdata=df_boot, type="response")
  df_boot$ps4 <- predict(model_s4,newdata=df_boot, type="response")
  df_boot$ps5 <- predict(model_s5,newdata=df_boot, type="response")
  
  # estimate E[Y^a | R = 0] and E[S^a | R = 0]
  ey1 <- sum(df_boot[df_boot$A==1,]$Y) / (nrow(df_boot[df_boot$A==1,]))
  es1 <- sum(df_boot[df_boot$A==1,]$S) / (nrow(df_boot[df_boot$A==1,]))
  
  n_r <- sum(1 - df_boot$R)
  ey0 <- (1 / n_r) * sum((1-df_boot$R)*df_boot$py0 )
  ey2 <- (1 / n_r) * sum((1-df_boot$R)*df_boot$py2 )
  ey3 <- (1 / n_r) * sum((1-df_boot$R)*df_boot$py3 )
  ey4 <- (1 / n_r) * sum((1-df_boot$R)*df_boot$py4 )
  ey5 <- (1 / n_r) * sum((1-df_boot$R)*df_boot$py5 )
  
  es0 <- (1 / n_r) * sum((1-df_boot$R)*df_boot$ps0 )
  es2 <- (1 / n_r) * sum((1-df_boot$R)*df_boot$ps2 )
  es3 <- (1 / n_r) * sum((1-df_boot$R)*df_boot$ps3 )
  es4 <- (1 / n_r) * sum((1-df_boot$R)*df_boot$ps4 )
  es5 <- (1 / n_r) * sum((1-df_boot$R)*df_boot$ps5 )
  
  tau <- c(1 - ey1 / ey0, 1 - ey2 / ey0, 1 - ey3 / ey0, 1 - ey4 / ey0, 1 - ey5 / ey0)
  delta <- c(es1 - es0, es2 - es0, es3 - es0, es4 - es0, es5 - es0)
  rho_s_boot[i] <- cor(tau, delta, method = "spearman")
  i <- i + 1
}

rho_s_boot_notna <- rho_s_boot[!is.na(rho_s_boot)]
sd_rho_s <- sd(rho_s_boot_notna)

u <- c(alpha/2, 1-alpha/2)
rho_s_lower_percentile <- quantile(rho_s_boot, u)[1]
rho_s_upper_percentile <- quantile(rho_s_boot, u)[2]



# variance estimation for rho_p and beta ----------------------------------
